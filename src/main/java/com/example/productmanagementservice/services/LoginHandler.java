...
public class LoginHandler {
    ...
    @Autowired
    public LoginHandler(DatabaseHandler databaseHandler, VerificationDatabase verificationDatabase, CreatorInDatabase creatorInDatabase) {
    ...
    public ResponseEntity<Token> login(String login, String password){
        ...
        if(verificationDatabase.checkingUser(login,password)) {
            responseEntity = new ResponseEntity<>
                    (new Token(creatorInDatabase.createToken(login)),HttpStatus.OK);
        } else {
